...
class Base(models.AbstractModel):
    ...
    @api.model
    def _get_external_id(self, record, xml_ids):
    ...
    def _xmlify_record(
        ...
        xml = E.record(id=self._get_external_id(record, xml_ids), model=record._name)
    ...
    def _xmlify_field(
        ...
        if field.type in ["many2one", "many2many", "one2many"]:
            ...
            if field.type == "many2one":
                field_dict["ref"] = self._get_external_id(value, xml_ids)
            else:
                ids = [self._get_external_id(r, xml_ids) for r in value]
